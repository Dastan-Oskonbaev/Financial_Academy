...
class Teacher(models.Model):
    ...
    photo = models.ImageField(
        _("Фотография"), upload_to="teachers/", default="default_image.jpg"
    )
...
class TeacherImage(models.Model):
    images = models.ImageField(
        _("Изображения"), upload_to="teachers_images/", default="default_image.jpg"
    )
...
class Course(models.Model):
    ...
    photo = models.ImageField(
        upload_to="courses/", verbose_name=_("Фото"), default="default_image.jpg"
    )
    ...
    number_of_exercises = models.CharField(
        _("Количество занятий"), max_length=100, null=True, blank=True, default="test"
    )
    number_of_students = models.CharField(
        _("Количество студентов"), max_length=100, null=True, blank=True, default="test"
    )

    price = models.CharField(
        _("Цена"), max_length=100, null=True, blank=True, default="test"
    )
    visiting_days = models.CharField(
        _("Дни посещения (Пн, Ср, Пт)"),
        max_length=100,
        null=True,
        blank=True,
        default="test",
    ...
    profession = models.CharField(
        _("Профессия"), max_length=100, null=True, blank=True, default="test"
    )
    skills = models.ManyToManyField(
        "CourseSkill", blank=True, verbose_name=_("Список навыков (макс. 5)")
    )
    main_objective = models.TextField(
        _("Главная цель"), null=True, blank=True, default="test"
    )
...
class ProfessionalAdvantage(models.Model):
    ...
    logo = models.ImageField(
        upload_to="advantages/", verbose_name=_("Логотип"), default="default_image.jpg"
    )

    class Meta:
        verbose_name = _("Преимущество (Курс)")
        verbose_name_plural = _("Преимущества (Курс)")
...
class NewsImage(models.Model):
    images = models.ImageField(
        _("Изображения"), upload_to="news/", default="default_image.jpg"
    )
...
class Review(models.Model):
    ...
    def clean(self):
        ...
        if self.is_displayed and displayed_count > 5:
            raise ValidationError(
                {
                    "is_displayed": "Нельзя добавить больше 5 отображаемых отзывов на главной странице."
                }
            )
...
class Advantage(models.Model):
    ...
    preview_body = models.CharField(
        max_length=255, verbose_name=_("Краткое описание"), null=True, blank=True
    )
    body = models.TextField(verbose_name=_("Текст"), max_length=300)
    logo = models.ImageField(
        upload_to="advantages/", verbose_name=_("Логотип"), default="default_image.jpg"
    )
    is_displayed = models.BooleanField(
...
class OutsourcingService(models.Model):
    image = models.ImageField(
        _("Картинка"), upload_to="our_services/", default="default_image.jpg"
    )
    ...
    class Meta:
        verbose_name = _("Услуга (Аутсорсинга)")
        verbose_name_plural = _("Услуги (Аутсорсинг)")
